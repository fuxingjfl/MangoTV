...
import com.xha.mangotv.adapter.TimeAdapter;
 ...
 * 
 */
public class CalendarLsInOutPop extends PopupWindow {
	private int resId;
	...
	public CalendarLsInOutPop(Context context, int resId
	...
	public void initPopupWindow() {
		...
		for (int i=0;i<23;i+=2){
			...
			if (start_hour.length()<2){
				start_time="0"+start_hour+":00:00";
			}else{
				start_time=start_hour+":00:00";
			}
			if (end_hour.length()<2){
				end_time="0"+end_hour+":59:59";
			}else{
				end_time=end_hour+":59:59";
			}
			mlist.add(start_time+"-"+end_time);
		}
		...
		ll_bg.setOnClickListener(onClickListener);
		...
		public void onClick(View v) {
			switch (v.getId()){
				case R.id.single_day:
					...
					gv_sjd.setVisibility(View.GONE);
					gridCalendarView.setVisibility(View.VISIBLE);
...
//					addFragments(dayDuanFragment);
			...
			time_duan_view.setVisibility(View.VISIBLE);
				gv_sjd.setVisibility(View.VISIBLE);
			gridCalendarView.setVisibility(View.GONE);

				if (timeAdapter==null){

					timeAdapter = new TimeAdapter(context,mlist);

					timeAdapter.setOnItemClickListener(itemClickListener);

					gv_sjd.setAdapter(timeAdapter);
				}
	...
	private TimeAdapter.ItemClickListener itemClickListener = new TimeAdapter.ItemClickListener() {
		@Override
		public void OnItemClickListener(String time,int p) {
			if (p==0){
