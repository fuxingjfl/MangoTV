...
public class ContrastChannelAdapter extends BaseQuickAdapter <ContrastChannelData, BaseViewHolder> {
    ...
    public ContrastChannelAdapter(String channelCode,@LayoutRes int layoutResId, @Nullable List<ContrastChannelData> data,List<Boolean> zt,Context context,int count) {
...
//        this.count=count;
                ...
                public void onClick(View v) {
                    if(helper.getPosition()<zt.size()){
                        ...
                        if(zt.get(helper.getPosition())){//选中
                            ...
                            Log.e("TAG","数据cou==="+ AddContrastChannelActivity.count);
                            if(AddContrastChannelActivity.count<3){
                                rl_pdmc.setBackgroundResource(R.drawable.shape_rl_xz);
                                tv_pd_name.setTextColor(context.getResources().getColor(R.color.black));
    ...
    public void setState(List<Boolean> zt){
        this.zt=zt;
    }
}
