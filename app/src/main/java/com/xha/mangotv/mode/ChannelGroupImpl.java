...
public class ChannelGroupImpl implements DataModel,OkHttpClientManager.HttpCallback {
	private onDataListener onDataListener;
	@Override
	public void onSuccess(String result, int code) {
		// TODO Auto-generated method stub
		
		Log.e("TAG", "频道组=="+result);
		...
		try {
			...
			case HttpConstants.search_news01:
				...
				if(jo1.getBoolean("success")){
					...
					Info info = new Info();
					info.channelGroups = new ArrayList<>();
					for(int i=0;i<ja.length();i++){

						JSONObject joo = ja.getJSONObject(i);
						ChannelGroup channelGroup = new ChannelGroup();
						channelGroup.areaCode=joo.getString("areaCode");
						channelGroup.channelGroupCode=joo.getString("channelGroupCode");
						channelGroup.channelGroupName=joo.getString("channelGroupName");
						channelGroup.orderNo=joo.getString("orderNo");
						info.channelGroups.add(channelGroup);
					}
