...
public abstract class MonthView extends View {
    ...
    protected int xzYear,xzMonth,xzDay;
    protected int twoYear,twoMonth,twoDay;
    ...
    public MonthView(Context context, AttributeSet attrs) {
        ...
        setendSelectDate(currYear,currMonth,currDay);
        setLeftDate();
        setRightDate();
        createTheme();
    ...
    private void drawDate(Canvas canvas,int year,int month,int startX,int startY){
        ...
        Log.e("TAG","年====="+year+",month==="+month);
        for(int day = 0;day < mMonthDays;day++){
            column = (day+weekNumber - 1) % 7;
            row = (day+weekNumber - 1) / 7;
            daysString[row][column]=day + 1;
            if (isyl){
                drawEndBG(canvas,column,row,daysString[row][column],year,month);
            }else{
                drawBG(canvas,column,row,daysString[row][column],year,month);
            }
    ...
    protected abstract void drawBG(Canvas canvas,int column,int row,int day,int year,int month);

    protected abstract void drawEndBG(Canvas canvas,int column,int row,int day,int year,int month);
        ...
        switch(eventCode){
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(upX-downX > 0 && Math.abs(upX-downX) > mTouchSlop*10){//左滑
                    ...
                    if(isyl){
                        if(isdj){
                            ...
                            doendClickAction((upX + downX)/2,(upY + downY)/2);
                        }
                    }else{
                        doClickAction((upX + downX)/2,(upY + downY)/2);
                    }
                ...
                if(smoothMode == 0) {
                    ...
                    Log.e("TAG","indexMonth==="+indexMonth);
    ...
    protected void setSelectDate(int year,int month,int day){
        Log.e("TAG","选中的第一个点year=="+year+",month=="+month+",day="+day);
        selYear = year;
        selMonth = month;
        selDay = day;
    }
    ...
    protected int endselYear,endselMonth,endselDay;
    /**
     * 设置选中的结束月份
     * @param year
     * @param month
     */
    protected void setendSelectDate(int year,int month,int day){
        Log.e("TAG","选中的第二个点year=="+year+",month=="+month+",day="+day);
        endselYear = year;
        endselMonth = month;
        endselDay = day;
    }
     ...
     * @param y
     */
    private void doendClickAction(int x,int y){
        int row = (int) (y / rowSize);
        int column = (int) (x / columnSize);
        twoYear=selYear;
        twoMonth=selMonth;
        twoDay=daysString[row][column];

//        Log.e("TAG","twoYear==="+twoYear+",twoMonth=="+twoMonth+",twoDay===="+twoDay);

        setendSelectDate(endselYear,endselMonth,daysString[row][column]);
        invalidate();
        //执行activity发送过来的点击处理事件
        if(dateClick != null){
            dateClick.onEndClickOnDate(endselYear,twoMonth + 1,endselDay);
        }
        isdj=true;
    }
    ...
    private void setLeftDate(){
        ...
        Log.e("TAG","左滑中:::year=="+year+",month="+month+",day="+day);

//        if (isyl){
//            setendSelectDate(year,month,day);
//        }else{
...
//        }
    ...
    private void setRightDate(){
        ...
        Log.e("TAG","右滑中:::year=="+year+",month="+month+",day="+day);
...
//        }
    ...
    public void setDateClick(IDateClick dateClick,boolean isyl) {
        this.dateClick = dateClick;
        Log.e("TAG","是否两个呢~~~~~~~~~~~~`"+isyl);
        this.isyl=isyl;
        invalidate();
    }
    ...
    public interface IDateClick{
        ...
        void onEndClickOnDate(int year,int month,int day);
    }
